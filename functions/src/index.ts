...
export const createUserAU = functions.region(REGION).https.onCall(async (data, context) => {
    ...
    const callerIsAdmin = callerData?.isAdmin === true;
    const isAuthorized = callerRole === 'teacher' || callerRole === 'admin' || callerIsAdmin;

    if (!isAuthorized) {
        throw new functions.https.HttpsError(
            'permission-denied',
            'Only teachers and admins can create users.'
        );
    }
    ...
    if (!email || !password || !firstName || !surname || !targetRole) {
        throw new functions.https.HttpsError(
            'invalid-argument',
            'Missing required fields: email, password, firstName, surname, role'
        );
    }
    ...
    try {
        ...
        const userDoc = {
            ...
            ...(targetRole === 'student' && {
                ...
                currentLevel: 1,
                aiApproved: false,
                password: password, // Store password for teacher reference
        ...
        if (error.code === 'auth/email-already-exists') {
            try {
                ...
                if (!userSnap.exists) {
                    ...
                    return { id: existingUser.uid, ...userDoc };
                } else {
                    ...
                    return { id: existingUser.uid, ...userSnap.data() };
                }
...
async function ensureTeachersClass() {
    ...
    if (!teachersClassSnap.exists) {
        await teachersClassRef.set({
            name: 'Teachers',
            teacherId: 'system', // System-owned class
            yearLevel: 0,
            studentIds: []
        });
    }
...
async function createFakeStudentAccount(teacherData: any, password: string) {
    ...
    try {
        ...
        await admin.firestore().collection('users').doc(userRecord.uid).set({
            ...
            role: 'student',
            classIds: [classId],
            yearLevel: 7, // Default for testing
            locked: false,
            enrolled: true,
            currentLevel: 1,
...
export const syncUserClaimsAU = functions.region(REGION).https.onCall(async (data, context) => {
...
export const onUserClassChange = functions.region(REGION).firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const beforeClasses = (beforeData?.classIds || []) as string[];
        const afterClasses = (afterData?.classIds || []) as string[];
        ...
        for (const classId of classesToAdd) {
            ...
            promises.push(
                admin.firestore().collection('classes').doc(classId).update({
...
export const onUserRoleChange = functions.region(REGION).firestore
    .document('users/{userId}')
    .onWrite(async (change, context) => {
        const userId = context.params.userId;
        ...
        const oldIsAdmin = beforeData?.isAdmin === true;
...
export const changePasswordAU = functions.region(REGION).https.onCall(async (data, context) => {
